...
def view_all():
    lt1.delete(0,END)
    for row in Backhend_Mysql.view():

        lt1.insert(END,row)

def search_command():
    lt1.delete(0, END)
    for row in Backhend_Mysql.Search(Title_text.get(),Year_text.get(),Author_text.get(),ID_text.get()):
        lt1.insert(END, row)

def add_command():

    Backhend_Mysql.insert(Title_text.get(), Year_text.get(), Author_text.get(), ID_text.get())
    lt1.delete(0, END)
    lt1.insert(END, (Title_text.get(), Year_text.get(), Author_text.get(), ID_text.get()))


def delete_command():
    Backhend_Mysql.delete(select_tuple[0])

def update_command():
    Backhend_Mysql.update(select_tuple[0],Title_text.get(), Year_text.get(), Author_text.get(), ID_text.get())
